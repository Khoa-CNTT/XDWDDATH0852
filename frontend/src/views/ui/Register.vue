<template>
  <div class="max-w-md mx-auto bg-white rounded-lg shadow-md overflow-hidden mt-10">
    ...
    <form @submit.prevent="handleRegister" class="p-6">
      ...
      <div class="mb-4">
        <label for="fullname" class="flex items-center gap-1 text-gray-700 font-medium mb-2"><i class='bx bx-user'></i>
          Nhập họ tên</label>
      ...
      <div class="mb-4">
        <label for="email" class="flex items-center gap-1 text-gray-700 font-medium mb-2"><i class='bx bx-envelope'></i>
          Nhập email</label>
      ...
      <div class="mb-4">
        <label for="address" class="flex items-center gap-1 text-gray-700 font-medium mb-2"><i class='bx bx-map'></i>
          Nhập địa chỉ</label>
      ...
      <div class="mb-6">
        <label for="confirmPassword" class="flex items-center gap-1 text-gray-700 font-medium mb-2"><i
            class='bx bx-lock'></i> Xác nhận mật khẩu</label>
...
const handleRegister = async () => {
  ...
  // Validate fullName
  const removeInvalidChars = /[^A-Za-zÀ-ỹà-ỹ\s]/gu
  let inputFullName = fullname.value.replace(removeInvalidChars, '')

  inputFullName = inputFullName.replace(/([\p{Ll}])([\p{Lu}])/gu, '$1 $2')
  inputFullName = inputFullName.replace(/\s+/g, ' ').trim()

  fullname.value = inputFullName

  if (fullname.value.length <= 2) {
    error.value = 'Họ tên quá ngắn.'
    loading.value = false
    return
  }
  ...
  if (!gmailRegex.test(email.value)) {
    error.value = 'Email đúng định dạng @gmail.com'
    loading.value = false
    return
  }
  ...
  if (phone_number.value.length !== 10) {
    error.value = 'Số điện thoại không đủ 10 chữ số'
    loading.value = false
    return
  }
  ...
  if (password.value.length > 0 && password.value.length < 8) {
    error.value = 'Mật khẩu phải đủ 8 ký tự'
    loading.value = false
    return
  }
